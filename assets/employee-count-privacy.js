...
function jwtSub(token){try{let p=String(token||'').split('.')[1]||'';if(!p)return'';p=p.replace(/-/g,'+').replace(/_/g,'/');while(p.length%4)p+='=';return JSON.parse(atob(p)).sub||''}catch(_){return''}}
