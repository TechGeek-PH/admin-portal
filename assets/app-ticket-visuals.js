...
function decorateCard(card){if(!(card instanceof Element))return;const issue=card.querySelector('.issue'),detail=card.querySelector('.detail');const combined=[issue&&issue.textContent,detail&&detail.textContent].filter(Boolean).join(' ');const kind=classify(combined);const nextClass=kind?'tg-issue-'+kind:'';['critical','warning','install','relocate','repair','cctv','billing'].forEach(k=>{const c='tg-issue-'+k;if(c!==nextClass&&card.classList.contains(c))card.classList.remove(c)});if(nextClass&&!card.classList.contains(nextClass))card.classList.add(nextClass);const acct=accountFrom(card);let badge=card.querySelector('.tg-app-account');if(!acct){if(badge)badge.remove();return}if(!badge){badge=document.createElement('div');badge.className='tg-app-account';const chips=card.querySelector('.chips');if(chips)card.insertBefore(badge,chips);else if(issue)issue.insertAdjacentElement('afterend',badge);else card.appendChild(badge)}const current=badge.querySelector('b')&&badge.querySelector('b').textContent||'';if(current!==acct)badge.innerHTML='<span>Account No.</span><b>'+esc(acct)+'</b>'}
...
function start(){ensureCss();apply();const root=document.getElementById('ticketRecords');if(root)new MutationObserver(schedule).observe(root,{childList:true});window.addEventListener('message',e=>{if(e.origin===location.origin&&e.data&&e.data.type==='tg-clients-changed')setTimeout(schedule,300)});window.addEventListener('storage',e=>{if(e.key==='tg_clients_changed_at')setTimeout(schedule,300)})}
