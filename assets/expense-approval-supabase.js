...
function isCompanyExpense(row){return !!(row&&(row.payroll_deductible===false||/\bcompany expense\b/i.test(String(row.remarks||''))))}
...
async function updateStatus(id,requested){
  ...
  const verb=company?'release as COMPANY EXPENSE (no payroll deduction)':next==='Approved'?'approve':next==='Released'?'mark as released and include eligible Cash Advance/Food in payroll deduction':'reject';
  if(!confirm('Confirm: '+verb+' '+id+' for '+row.employee_name+' ('+money(row.amount)+')?'))return;
  const def=company?'Company Expense — Released directly; not payroll deductible.':next==='Approved'?'Approved by admin. Waiting for release before payroll deduction.':next==='Released'?'Released / paid by admin. Eligible Cash Advance/Food will now be deducted in this cutoff.':'Rejected by admin.';
  ...
  try{
    ...
    if(next==='Approved'){payload.payroll_deductible=true;payload.approved_by=profile.full_name;payload.approved_at=now;payload.released_by=null;payload.released_at=null}
    if(next==='Released'){payload.payroll_deductible=!company;payload.released_by=profile.full_name;payload.released_at=now;if(company){payload.approved_by=null;payload.approved_at=null}}
    if(next==='Rejected'){payload.released_by=null;payload.released_at=null}
    const d=await api('/rest/v1/app_expenses?expense_id=eq.'+encodeURIComponent(id),{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)});if(!Array.isArray(d)||!d.length)throw new Error('Expense record was not updated.');await load(true);note(id+' updated to '+next+(company?' as Company Expense — no payroll deduction.':next==='Released'&&['cash advance','food'].includes(norm(row.category))?' — included in payroll deduction for its cutoff.':'.'),'ok');
...
function exportCsv(){const view=filtered(),h=['Expense ID','Date','Employee ID','Employee Name','Category','Amount','Purpose','Status','Payroll Deductible','Remarks','Approved By','Released By'];const a=[h].concat(view.map(r=>[r.expense_id,r.expense_date,r.employee_id,r.employee_name,r.category,r.amount,r.purpose,r.status,r.payroll_deductible,r.remarks,r.approved_by,r.released_by]));const csv=a.map(line=>line.map(v=>'"'+String(v==null?'':v).replace(/"/g,'""')+'"').join(',')).join('\n');const b=new Blob([csv],{type:'text/csv'}),u=URL.createObjectURL(b),x=document.createElement('a');x.href=u;x.download='expense_approval_'+new Date().toISOString().slice(0,10)+'.csv';document.body.appendChild(x);x.click();x.remove();URL.revokeObjectURL(u)}
function bind(){
