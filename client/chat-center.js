...
function init(){
  ...
  @media(max-width:520px){.nav button{font-size:.53rem}.tg-chat-tools{grid-template-columns:1fr}.tg-chat-ticket{min-height:42px}.tg-msg{max-width:92%}.tg-chat-hint{max-width:60%}}
  `;
  ...
  const pane=document.createElement('section');pane.id='chatPane';pane.className='pane';pane.innerHTML=`<section class="section tg-chat-shell"><div class="tg-chat-head"><div class="tg-chat-title"><h2>Chat Center</h2><span class="tg-chat-online">Telegram Support Online</span></div><div class="tg-chat-account">Conversation for <b id="tgChatAccount">your account</b></div><div class="tg-chat-tools"><select id="tgChatCategory" aria-label="Concern category"><option>No Internet</option><option>Red LOS</option><option>Slow Internet</option><option>Intermittent Connection</option><option>Billing Concern</option><option>Payment Concern</option><option>Router / Wi-Fi</option><option>Installation</option><option>Other Concern</option></select><button type="button" id="tgChatCreateTicket" class="tg-chat-ticket">Create Support Ticket</button></div></div><div id="tgChatError" class="tg-chat-error"></div><div id="tgChatMessages" class="tg-chat-messages"><div class="tg-chat-empty">Loading your support conversation…</div></div><div class="tg-chat-compose"><textarea id="tgChatText" maxlength="1500" placeholder="Type your concern here. TechGeekPH Support will receive this in Telegram."></textarea><div class="tg-chat-actions"><div class="tg-chat-hint">Replies from support will appear here automatically.</div><button type="button" id="tgChatSend" class="tg-chat-send">Send Message</button></div></div></section>`;
  ...
  function render(r){
    $('tgChatAccount').textContent=(r.account_no||'Account')+(r.client_name?' · '+r.client_name:'');setUnread(r.unread||0);
    ...
    if(!rows.length){messages.innerHTML='<div class="tg-chat-empty"><b>No messages yet.</b><br>Send your concern and our support team will receive it through Telegram.</div>';return}
    messages.innerHTML=rows.map(x=>{const type=String(x.sender_type||'SYSTEM').toLowerCase(),who=x.sender_type==='CLIENT'?'You':(x.sender_name||'TechGeekPH Support');return `<div class="tg-msg ${esc(type)}"><div class="tg-msg-meta">${esc(who)} · ${esc(fmt(x.created_at))}</div><div class="tg-msg-bubble">${x.category?`<div class="tg-msg-category">${esc(x.category)}</div>`:''}${esc(x.message_body)}</div></div>`}).join('');
  ...
  async function send(){
    ...
    try{const {data:r,error}=await db.functions.invoke('client-chat-send',{body:{token:token(),category:$('tgChatCategory').value,message:text}});if(error)throw error;if(!r?.ok)throw new Error(r?.message||'Unable to send message.');$('tgChatText').value='';await loadChat(true)}catch(e){showError(e?.message||'Unable to send message right now.')}finally{busy=false;$('tgChatSend').disabled=false;$('tgChatSend').textContent='Send Message'}
  }
  ...
  $('tgChatCreateTicket').addEventListener('click',()=>document.getElementById('newTicketBtn')?.click());
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)loadChat(chatActive())});
  setInterval(()=>{if(token()&&!document.hidden)loadChat(chatActive())},12000);
  setTimeout(()=>loadChat(false),500);
}
